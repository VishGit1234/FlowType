...
class Typer extends Component {
  ...
  render() {
    ...
    if (!started)
      return (
        <div className="border-2 border-white border-solid text-white p-2 text-xl">
          <h2>Welcome to the Typing game</h2>
          <p>
            <strong>Rules:</strong> <br />
            Type in the input field the highlighted word. <br />
            The correct words will turn <span className="text-[#4ade80]">green</span>. <br/>
            Incorrect letters will turn <span className="text-[#f43f5e]">red</span>. <br />
            Good Luck! <br/>
            <button className="mx-40" onClick={this.startGame}>
            Start game
            </button>
          </p>
        </div>
    ...
    if (completed) {
      return (
        <div className="border-2 border-white border-solid text-white p-2 text-xl">
          ...
          <button className="start-btn pl-4" onClick={this.endGame}>
            Submit Score
          </button>
        </div>
      );
    ...
    return (
      <div className="border-2 border-white border-solid text-white p-2 text-xl w-2/3">
        <div className="wpm">
          <strong>WPM: </strong>
          {wpm}
          <br />
              ...
              return (
                <span
                  className={`text-lg mx-0.5 
                                ${highlight && "text-[#f43f5e]"} 
                                ${currentWord && "border-b-1 border-solid border-black"}`}
                  key={w_idx}
                >
                  {word.split("").map((letter, l_idx) => {
                    const isCurrentWord = w_idx === completedWords.length;
                    const isWronglyTyped = (letter !== inputValue[l_idx]) && (letter !== " ");
                    const shouldBeHighlighted = l_idx < inputValue.length;
                    // console.log(`wrong: ${isWronglyTyped}`)
                    return (
                      <span
